...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            const tabunganInput = document.getElementById('tabungan');
            ...
            const hartaLainInput = document.getElementById('hartaLain');
            ...
            tabunganInput.addEventListener('input', function() {
                updateInputValue(this);
            });

            emasInput.addEventListener('input', function() {
                updateInputValue(this);
            });

            sahamInput.addEventListener('input', function() {
                updateInputValue(this);
            });

            realEstateInput.addEventListener('input', function() {
                updateInputValue(this);
            });
            ...
            hartaLainInput.addEventListener('input', function() {
                updateInputValue(this);
            });
            ...
            hargaEmasInput.addEventListener('input', function() {
                updateInputValue(this, false); // Do not include decimals for hargaEmasInput
                if (this.value === 'Rp. 0') {
            ...
            document.querySelector('.button').addEventListener('click', function() {
                const tabungan = parseFloat(tabunganInput.value.replace(/\D/g, '')) || 0;
                const emas = parseFloat(emasInput.value.replace(/\D/g, '')) || 0;
                const saham = parseFloat(sahamInput.value.replace(/\D/g, '')) || 0;
                const realEstate = parseFloat(realEstateInput.value.replace(/\D/g, '')) || 0;
                const mobil = parseFloat(mobilInput.value.replace(/\D/g, '')) || 0;
                const hartaLain = parseFloat(hartaLainInput.value.replace(/\D/g, '')) || 0;
                ...
                const total = tabungan + emas + saham + realEstate + mobil + hartaLain - hutangCicilan;
                const nisabZakat = 85 * hargaEmas;
                const amount = 0.025 * total;

                document.getElementById('tot').value = formatToCurrency(total);
                document.getElementById('nisab').value = formatToCurrency(nisabZakat, false);

                if (isNaN(nisabZakat) || nisabZakat <= 0) {
                    document.getElementById('gold').style.display = 'block';
